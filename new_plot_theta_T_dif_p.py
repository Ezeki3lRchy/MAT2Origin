...
mat_data = loadmat("H:/postgraduate/matlab/newdata_theta_T_dif_p.mat")
...
wks_1.from_list(0, mat_data['x_data'], units='K', lname='T', axis='X')
...
for i, param in enumerate(params):  # the number of i is from 0
    ...
    if y_data_key in field_names:
        wks_1.from_list(i + 1, list(gamma_data_struct[0, 0][y_data_key]),
                        units='', lname=f'{param_int}Pa ', axis='Y')
    else:
...
op.save(op.path('u') + 'new_plot_theta_T_dif_p.opju')
